...
- **Escalate to `wasm-worker-process` only on the runtime-escape axis**, which is orthogonal to tenant count, decided on two concrete conditions:
  ...
  - *Secret co-residency* — the host process holds high-value data the guest must never reach (API keys, other tenants' data) **and** the guest is hostile. Spectre-class speculation can leave a secret-dependent cache footprint regardless of the guest's capabilities, because mis-speculation happens in silicon below the WASM sandbox; reading that footprint out, however, requires a timer. The default configuration denies the guest one entirely: no clock capability, and — unlike a browser — no `SharedArrayBuffer` or thread primitive inside the wasm sandbox, so a guest cannot build a counter-thread timer (the timeout mechanism's SAB is host-side adapter state, never exposed to guest JS; see CPU And Timeouts). This makes timing side channels *hard* but not impossible: amplification across many gadget repetitions can resolve cache state from coarse residual signals (loop counts, host round-trip latency), and a deployment that exposes any timing-correlated host callback — or enables guest threads/SAB for performance — reintroduces a timer. So timer-denial is a strong, configuration-dependent mitigation, not a boundary. The only categorical control is address-space separation: a secret that does not share an address space with hostile speculation cannot be Spectre-leaked from it. Move the guest out, or move the secrets out. (Semi-trusted guests — the default target — do not meet the "hostile" condition; this is a hostile-code concern, not a blanket one.)
