...
- Because of that, an `eval` value tree **may contain handle leaves**. The response descriptor signals this via its `tag` (the `value_with_handles` shape under `status = ok`; see Wire Codec → AbiResponse) so the host knows the result carries handles it now **owns and must dispose**. Handle-leaf count is bounded by the configured max-handle-count cap. A pure-primitive/tree result uses the plain `value` shape, carries no handles, and needs no disposal.
...
- Clock and randomness are policy points: the shim must support coarsening or fixing `clock_time_get` and deterministic `random_get` for deployments that want to withhold timing capabilities from guest JS. Combined with the absence of a guest `SharedArrayBuffer`/thread primitive, this denies the default guest a high-resolution timer (the timeout SAB is host-side, not guest-reachable). Strong against timing side channels, but not a Spectre boundary — amplification works from coarse residual signals, and the property is configuration-dependent (a timing-correlated host callback reintroduces a timer). See Deployment Profiles, secret co-residency.
...
- Import attributes (`import x from "y" with { type: "json" }`) are passed through to the resolver but **do not contribute to canonical-key identity** (decided 2026-06-12). rquickjs 0.12's `Loader` trait surfaces them, so the `ModuleRequest` event carries them alongside `specifier`/`referrer_key` and the resolver may act on them (treating them as untrusted, like the specifier) — but the same `(specifier, referrer_key)` always resolves to the same canonical key regardless of attributes. This keeps module identity stable: `import "x"` and `import "x" with {...}` are the same module instance, matching JS host semantics, and avoids attributes becoming a guest-controlled cache-keying lever.
...
- **Escalate to `wasm-worker-process` only on the runtime-escape axis**, which is orthogonal to tenant count, decided on two concrete conditions:
  - *Defense-in-depth is explicitly required for this workload* — you have decided a single sandbox is not sufficient (regulated data, a compliance mandate, or code you have no legal recourse against), so you add the process as a second, independently-failing containment layer in case of a WASM-runtime escape. This is not a default assumption that Wasmtime will be escaped — it is among the most-hardened sandboxes in production and is this project's chosen boundary; the process you would escalate to is itself just another sandbox with its own CVE surface. Treat this as a deliberate risk-tolerance call for high-assurance workloads, not a baseline. It is a function of assurance requirements, not of how many tenants you serve.
  - *Secret co-residency* — the host process holds high-value data the guest must never reach (API keys, other tenants' data) **and** the guest is hostile. The headline risk is Spectre-class speculation leaving a secret-dependent cache footprint below the WASM sandbox. In practice the default configuration makes this remote: reading the footprint out needs a timer, and the default guest has none — no clock capability, and (unlike a browser) no `SharedArrayBuffer` or thread primitive in the sandbox to build a counter-thread timer (the timeout SAB is host-side, never exposed to guest JS). Cache exfiltration is also intrinsically slow and needs sustained gadget iteration, which the wall-clock timeout and instance recycling cap. The residual is a narrow, configuration-dependent case (a deployment that exposes a timing-correlated host callback, or enables guest threads/SAB), and it is mentioned only because the one *categorical* control is address-space separation — which is the reason this profile exists. For high-assurance deployments that cannot accept a configuration-dependent mitigation, move the guest out or move the secrets out. (The default semi-trusted target does not meet the "hostile" condition and need not consider this at all.)
