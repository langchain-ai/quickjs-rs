...
class Globals:
    ...
    - ``globals["x"]`` reads via JS property access on ``globalThis``
      and marshals the result. Missing keys return ``None``
      (JS: ``globalThis.missing`` is ``undefined``), not ``KeyError``.
    - ``globals["x"] = value`` writes via ``globalThis["x"] = value``.
      ``value`` may be a marshaled Python value or a
      :class:`Handle` on the same context.
    - ``"x" in globals`` is True if ``globalThis["x"]`` exists *and*
    ...
    - ``del globals["x"]`` is not supported (declares
    ...
    def __getitem__(self, key: str) -> Any:
        # Read via a short-lived child handle so the marshaler
        # runs with allow_opaque=False. Missing keys come back as
        # undefined → None at the root per 's depth-0 rule.
